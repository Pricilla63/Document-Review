...
# app = Flask(__name__)
# CORS(app, origins=["http://localhost:5173", "http://127.0.0.1:5173"])  # Explicitly allow Vite dev server
...
# =============================================================================
# NEW DIRECT PROCESSING ENDPOINT
# =============================================================================

@app.route('/api/process-invoice', methods=['POST'])
def api_process_invoice():
    """Direct invoice processing endpoint - upload and process in one step"""
    try:
        print("🚀 Received direct invoice processing request")
        ...
        if file and file.filename.lower().endswith('.pdf'):
            ...
            # Process the file immediately using your existing function
            result_data = enhanced_extract_invoice_data(temp_path)
            ...
            if result_data:
                print(f"✅ Extraction completed for: {file.filename}")
                return jsonify({
                    'success': True,
                    'data': result_data,
        ...
        print(f"❌ Direct invoice processing failed: {e}")
        return jsonify({
            'success': False, 
...
def health_check():
    ...
    return jsonify({
        ...
        "endpoints": {
            "upload": ["/upload-pdf", "/upload-multiple-pdfs"],
            "management": ["/list-uploaded-files", "/delete-file/<name>", "/clear-all-files"],
            "processing": ["/api/process-invoice", "/process-uploaded-files", "/process-folder", "/create-excel-from-json"],
            "info": ["/check-folders", "/health"]
        },
        "line_item_extraction": {
            "methods": ["Azure DI", "Text Pattern", "LLM Analysis"],
...
if __name__ == '__main__':
    ...
    print("   - Process uploaded files")
    print("\n🎯 NEW DIRECT PROCESSING ENDPOINT:")
    print("   - POST /api/process-invoice - Upload and process in one step")
    print("\n📊 Line Item Extraction:")
    print("   - Azure Document Intelligence (Primary)")
    print("   - Text pattern matching (Fallback)")
    print("   - LLM analysis (Complex cases)")
    print("\n🎯 API Endpoints:")
    print("   - POST /api/process-invoice - Upload and process single PDF")
    print("   - POST /upload-pdf - Upload single PDF")
    print("   - POST /upload-multiple-pdfs - Upload multiple PDFs")
    print("   - GET /list-uploaded-files - List uploaded files")
    print("   - DELETE /delete-file/<name> - Delete specific file")
